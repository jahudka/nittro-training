...
CREATE TABLE `homepage_panels` (
    ...
    `width` INT,
    `height` INT,
    `publish_from` DATE COMMENT '(DC2Type:date_immutable)',
    `publish_until` DATE COMMENT '(DC2Type:date_immutable)',
    CONSTRAINT `homepage_panels_pkey` PRIMARY KEY (`id`)
) ENGINE=InnoDb CHARACTER SET 'utf8mb4';

ALTER TABLE `homepage_panels` AUTO_INCREMENT = 11;
...
ALTER TABLE `menu_categories` AUTO_INCREMENT = 8;
...
CREATE TABLE `menu_items` (
   ...
   `position` INT NOT NULL,
   `name` VARCHAR(255) NOT NULL,
   `price` text NOT NULL COMMENT '(DC2Type:array)',
   CONSTRAINT `menu_items_pkey` PRIMARY KEY (`id`),
   CONSTRAINT `fk_70b2ca2a12469de2` FOREIGN KEY (`category_id`) REFERENCES `menu_categories`(`id`) ON DELETE CASCADE
) ENGINE=InnoDb CHARACTER SET 'utf8mb4';

CREATE INDEX `idx_70b2ca2a12469de2` ON `menu_items` (`category_id`);
ALTER TABLE `menu_items` AUTO_INCREMENT = 176;
